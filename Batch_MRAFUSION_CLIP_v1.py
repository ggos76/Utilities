...
perform_pansharp = perform_pansharp.lower()
if perform_pansharp in yes_validation_list:
    perform_pansharp = True

else:     
    perform_pansharp = False
...
# For the pansharpening to happend, both MS and PAN layers must be ingested.
if perform_pansharp is True and (import_MS or import_MS_Thermal) is False:
    print ("Error - The pansharpening process requires the ingestion of the Multispectral bands")
    print ('Set   import_MS = "yes" ')
    sys.exit()
if perform_pansharp is True and import_PAN is False:
    print ("Error - The pansharpening process requires the ingestion of the Panchromatic band")
    print ('Set   import_PAN = "yes" ')
    sys.exit()
...
if perform_pansharp is True :

    print ("\t")
    print ("-------------------------------------------------------------------------------------")
    print ("                        Image pansharpening using the MRAFUSION                      ")
    print ("-------------------------------------------------------------------------------------")
    print ("\t")

    nb_pansharp = str(len(out_PAN_list))
    out_mrafusion = []
    count = 1
    for pan_in, ms_in in zip (out_PAN_list, out_MS_list2):

        print ("\t")      
        print ((time.strftime("%H:%M:%S"))+" Pansharpening (MRAFUSION) file "+ str(count)+" of "+ nb_pansharp +"...")
        print ("   Panchromatic file --->" + pan_in)
        print ("   Multispectral file --->" + ms_in)
        ...
        outname = os.path.basename(pan_in[:-8])
        
        filo = os.path.join (output_folder, outname + "_mrafused.pix")
        dboc = pansharp_bands
...
if perform_clip is True :
    ...
    file_to_process = []
    if perform_pansharp is True :
        file_to_process = out_mrafusion      
    else:     
        file_to_process = out_PAN_list + out_MS_list
